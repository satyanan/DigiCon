...
class Word(object):
    ...
    def triple_typos(self):
        return {e3 for e2 in self.double_typos()
                for e3 in Word(e2).typos()}

def isEnglish(words):
    return set(words) & ENGLISH

def isMedicine(words):
    return (set(words) & MEDICINE) | (set(words) & MED_WORDS)

def isSymptom(words):
    return set(words) & SYMPTOMS
